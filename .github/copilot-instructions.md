...
React + .NET 10 API (Azure Static Web App / Azure App Service)
Code the react client app in such a way so that it is still functional if not API exists or it cannot connect to an API
Host the react app inside the .NET server app so only the server project needs to start to run the entire app
